...
3-4 j: tjjj"""
    ...
    def __init__(self):
        ...
        for i in input_list:
            policy = i.split(': ')[0]
            letters = policy.split(' ')[0].split('-')
            password = i.split(': ')[1]

            first_index = int(letters[0])
            second_index = int(letters[1])
            letter = policy[len(policy) - 1]

            valid_passwords += sum(password[index - 1] == letter for index in (first_index, second_index)) == 1
