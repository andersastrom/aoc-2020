...
seseseswseweseseswseesesesewswswnwese"""
    ...
    dirs = {'nw': (-1, 1, 0), 'w': (-1, 0, 1), 'sw': (0, -1, 1), 'se': (1, -1, 0), 'e': (1, 0, -1), 'ne': (0, 1, -1)}
    black_tiles = set()
    white_tiles = set()
    ...
    def __init__(self):
        ...
        for line in lines:
            # Axis: left to right, vertical, right to left
            cur = [0, 0, 0]
            i = 0
            length = len(line)
            for _ in range(length):
                ...
                if (line[i:i + 2] in self.dirs):
                    d = self.dirs[line[i:i + 2]]
                    i += 2
                else:
                    d = self.dirs[line[i]]
                    i += 1
            ...
            if cur in self.black_tiles:
                self.black_tiles.remove(cur)
            else:
                self.black_tiles.add(cur)

        self.generate_white_tiles(self.white_tiles)

        for i in range(100):
            new_black_tiles = self.black_tiles.copy()
            new_white_tiles = self.white_tiles.copy()
            
            self.flip_black_tiles(new_black_tiles, new_white_tiles)
            self.flip_white_tiles(new_black_tiles, new_white_tiles)

            self.black_tiles = new_black_tiles.copy()
            self.generate_white_tiles(new_white_tiles)
            self.white_tiles = new_white_tiles.copy()
        print(len(self.black_tiles))

    def generate_white_tiles(self, white_tiles):
        for bt in self.black_tiles:
            for d in self.dirs:
                adj = (bt[0] + self.dirs[d][0], bt[1] + self.dirs[d][1], bt[2] + self.dirs[d][2])
                if adj not in self.black_tiles:
                    white_tiles.add(adj)

    def flip_black_tiles(self, new_black_tiles, new_white_tiles):
        for bt in self.black_tiles:
            nr_of_adj_black = 0
            for d in self.dirs:
                adj = (bt[0] + self.dirs[d][0], bt[1] + self.dirs[d][1], bt[2] + self.dirs[d][2])
                if adj in self.black_tiles:
                    nr_of_adj_black += 1
            if nr_of_adj_black == 0 or nr_of_adj_black > 2:
                new_black_tiles.remove(bt)
                new_white_tiles.add(bt)

    def flip_white_tiles(self, new_black_tiles, new_white_tiles):
        for wt in self.white_tiles:
            nr_of_adj_black = 0
            for d in self.dirs:
                adj = (wt[0] + self.dirs[d][0], wt[1] + self.dirs[d][1], wt[2] + self.dirs[d][2])
                if adj in self.black_tiles:
                    nr_of_adj_black += 1
            if nr_of_adj_black == 2:
                new_black_tiles.add(wt)
                new_white_tiles.remove(wt)
