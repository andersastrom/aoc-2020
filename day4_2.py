...
ecl:amb"""
    ...
    required_fields = {"byr", "hcl", "hgt", "iyr", "eyr", "pid", "ecl"}

    def __init__(self):
        passports = self.input.split('\n\n')
        valid_passports = 0
        for passport in passports:
            fields = self._get_fields(passport)
            values = self._get_values(passport)
            if self._valid_passport(fields, values):
                valid_passports += 1

        print(valid_passports)

    def _get_fields(self, passport):
        return [field.split(':')[0] for field in passport.replace('\n', ' ').split(' ')]

    def _get_values(self, passport):
        return [field.split(':')[1] for field in passport.replace('\n', ' ').split(' ')]

    def _valid_passport(self, fields, values):
        if sum(field in self.required_fields for field in fields) == 7:
            return False not in [self._is_valid_field_value(field, values[i]) for i, field in enumerate(fields)]

    def _is_valid_field_value(self, field, value):
        if field == "byr":
            ...
            valid_format = re.compile("([0-9]+)([a-zA-Z]+)").match(value)
            if valid_format:
                items = valid_format.groups()
                unit = items[1]
                height = items[0]
                if unit == "cm":
            ...
            match = re.compile("(#[a-f0-9]+)").match(value)
            if match:
                return len(match.group()) == 7
            return False
